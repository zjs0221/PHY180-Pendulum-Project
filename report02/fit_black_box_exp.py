...
def load_data(filename):
    data = np.loadtxt(filename, delimiter=None, comments="#", skiprows=1, unpack=True)
    return data


def plot_fit(my_func, xdata, ydata, xerror, yerror, init_guess=None, font_size=14,
             xlabel="Indep Var (units)", ylabel="Dep Var (units)", 
             title="This Graph Needs a Better Title", color="black", labelA="Regression"):    
    ...
    ax1.errorbar(xdata, ydata, xerr=xerror, yerr=yerror, fmt=".", label=labelA, color="black")
    ...
    ax1.plot(xs, curve, label = rf"$\theta\left(t\right) = 0.175\, e^{{-\frac{{t}}{{{popt[0]:.1f}}}}} \cos\!\left(\frac{{2\pi t}}{{{popt[2]:.5f}}}\right)$", color=color)
    ...
    # --- Overlay exponential envelope (for damped sinusoid) ---
    # Calculate the exponential envelope: y = -0.175 * exp(-t/tau)
    envelope = -0.175 * np.exp(-xs / popt[0])
    
    # Overlay the curve in solid red
    ax1.plot(xs, envelope, 'r-', label = rf"$\theta\left(t\right) = \pm 0.175 e^{{-\frac{{t}}{{{popt[0]:.1f}}}}}$")
    
    # Optional: overlay the mirrored positive envelope for symmetry
    ax1.plot(xs, -envelope, 'r-')
    # -----------------------------------------------------------
    ...
    ax2.errorbar(xdata, residual, xerr=xerror, yerr=yerror, fmt=".", color="black")
